...
class Array2Xml {
	...
	public function __construct($args) {
		$array = $args[0];
		$rootElement = (empty($args[1]) ? "" : $args[1]);
		$replaceSpacesByUnderScoresInKeyNames = (empty($args[2]) ? true : $args[2]);
		$xmlEncoding = (empty($args[3]) ? null : $args[3]);
		$xmlVersion = (empty($args[4]) ? "1.0" : $args[4]);
		$domProperties = (empty($args[5]) ? [] : $args[5]);
		$xmlStandalone = (empty($args[6]) ? null : $args[6]);
